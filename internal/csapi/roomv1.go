...
func (a *API) RoomTimestampToEvent(w http.ResponseWriter, r *http.Request) {
	...
	// Not found locally: ask the room's remote servers (MSC3030 federation).
	// The found event is then backfilled so /context and /messages can serve it.
	if a.fed != nil {
		if exists, _ := a.Store.RoomExists(r.Context(), roomID); exists {
			for _, dest := range a.roomViaServers(r.Context(), roomID) {
				eventID, _, ferr := a.fed.Client().TimestampToEvent(r.Context(), dest, roomID, ts, dir)
				if ferr != nil {
					continue
				}
				a.backfillEventChain(r, dest, eventID)
...
func (a *API) backfillEventChain(r *http.Request, dest, eventID string) {
	...
	for depth := 0; depth < 10 && eventID != ""; depth++ {
		...
		// FetchRemoteEvent returns a PDU already verified by the remote; verify
		// its signature against the origin server's keys.
		_ = version
		row := &storage.EventRow{
			EventID: pdu.EventID, RoomID: roomID, Type: pdu.Type, Sender: pdu.Sender,
			Depth: pdu.Depth, OriginServerTS: pdu.OSTS, Content: pdu.Content, RawJSON: raw,
		}
		if pdu.StateKey != nil {
		...
		eventID = ""
		// Continue walking prev_events (the chain that leads back to the local
		// timeline, making back-pagination contiguous).
