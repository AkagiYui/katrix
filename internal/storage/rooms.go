...
func (s *Store) BackfillPoints(ctx context.Context, roomID string, limit int) ([]string, error) {
	...
	// The qualifying candidates, oldest (lowest depth) first. The oldest
	// qualifying events sit at the frontier of local knowledge — the point
	// where the room's history is missing — and asking the remote for history
	// before them is what fills the gap (mirror of Synapse's
	// get_backfill_points, which seeds from the oldest backward extremities
	// first).
	var out []candidate
	for _, c := range cands {
		missing := false
