...
//
// stateOverrides optionally replaces the default content of a state event the
// initial set always creates (keys "m.room.join_rules", "m.room.power_levels"
// and "m.room.history_visibility"). Room upgrade uses it to seed the new room
// with the predecessor's values from the start (so a client reading the first
// event of each type sees the copied value, per sytest), instead of emitting
// the default then a duplicate override later.
//
...
func BuildInitialEvents(
	...
	// stateOverrides may replace the defaults of the state events the initial
	// set always creates (room upgrade seeds the predecessor's values here).
	if o := stateOverrides["m.room.power_levels"]; len(o) > 0 {
		plRaw = o
	}
	...
	jrContent := InitialJoinRules(preset)
	if o := stateOverrides["m.room.join_rules"]; len(o) > 0 {
		jrContent = o
	}
	jrEv, err := buildAndSign(serverName, key, version, "m.room.join_rules", "", creator, roomID, jrContent, now, 3,
		[]string{plEv.EventID()}, append(plAuth, plEv.EventID()))
	if err != nil {
		return nil, err
	}

	hvContent := InitialHistoryVisibility(preset)
	if o := stateOverrides["m.room.history_visibility"]; len(o) > 0 {
		hvContent = o
	}
	hvEv, err := buildAndSign(serverName, key, version, "m.room.history_visibility", "", creator, roomID, hvContent, now, 4,
